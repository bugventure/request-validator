...
Validator.prototype.validate = function (schema, value) {
    ...
    var instance = this,
        ...
        multiValidator = this.getMultiValidator(schema),
        validators,
        failedValidators;

    if (multiValidator) {
        return multiValidator(schema, value, validateFunc);
    }

    validators = this.getValidators(schema);

    // if (!validators.length) {
    //     throw new Error(this.strings.validatorNotFound);
    // }
    ...
    validators.forEach(function forEachValidatorGroup(item) {
        item.funcs.forEach(function forEachValidationFunc(func) {
            ...
            try {
                func(schema, value, validateFunc);
            }
            catch (e) {
