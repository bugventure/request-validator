...
function validateProperties(schema, value, validator) {
    ...
    if (type.isObject(props)) {
        Object.keys(props).forEach(function forEachPropertyKey(key) {
            ...
            if (!type.isUndefined(nestedValue)) {
                // expect to throw
                validator(props[key]).validate(nestedValue);
            }
        });
    }
}

function validatePatternProperties(schema, value, validator) {
    var keys = Object.keys(value),
...
module.exports = function validateObject(schema, value, validator) {
    ...
    validateProperties(schema, value, validator);

    validatePatternProperties(schema, value, validator);
