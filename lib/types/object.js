...
Object.prototype.validateRequired = function (value) {
...
Object.prototype.isRequired = function (key) {
    return type.isArray(this.schema.required) &&
        this.schema.required.indexOf(key) > -1;
};

Object.prototype.validateProperties = function (value) {
    var that = this,
...
Object.prototype.validatePatternProperties = function (value) {
    var that = this,
        schema = that.schema,
        errors = that.errors,
        validator = that.validator,
    ...
    patterns.forEach(function forEachPattern(pattern) {
        ...
        matches.forEach(function forEachMatchingKey(key) {
            ...
            try {
                // expect to throw
                validator.validate(patternSchema, value[key]);
            }
            catch (e) {
                e.required = that.isRequired(key);
                e.missing = type.isUndefined(value[key]);
...
ObjectValidator.prototype.validateDependencies = function (value) {
    ...
    Object.keys(schema.dependencies).forEach(function forEachKey(key) {
        ...
        if (type.isObject(dependency)) {
            // schema dependency
            try {
                that.validator.validate(dependency, value);
            }
            catch (e) {
    ...
    if (type.isObject(schema.additionalProperties)) {
        keys.forEach(function forEachKey(key) {
            if (allowedKeys.indexOf(key) < 0) {
                ...
                // defined properties
                try {
                    validator.validate(schema.additionalProperties, value[key]);
                }
                catch (e) {
                    e.key = key;
                    e.missing = type.isUndefined(value[key]);
        ...
        // validate that object does not have excessive keys
