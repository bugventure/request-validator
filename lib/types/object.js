...
function validateDependencies(schema, value, validate) {
...
function validateAdditionalProperties(schema, value, validate) {
    if (type.isUndefined(schema.additionalProperties) ||
    ...
    if (type.isObject(schema.additionalProperties)) {
        keys.forEach(function forEachKey(key) {
            if (allowedKeys.indexOf(key) < 0) {
                ...
                // defined properties
                validate(schema.additionalProperties, value[key]);
            }
        });
    }
    else {
...
module.exports = function validateObject(schema, value) {
    ...
    validateDependencies(schema, value, validate);

    validateAdditionalProperties(schema, value, validate);
};